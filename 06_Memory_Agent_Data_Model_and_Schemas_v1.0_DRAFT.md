# Memory Agent — Data Model & Schemas v1.0
...
Esta regla evita que una futura solicitud de PURGE obligue a reescribir commits históricos que contienen plaintext personal.
...
# 19. Binding de payload sensible

Para valores Vault-backed, el Patch persistente no contiene plaintext.
...
# 20. Seguridad del Patch Hash con payload cifrado

El `patch_hash` no debe depender de un hash directo del plaintext sensible.

Para un valor Vault-backed, la representación canónica utiliza:
...
El binding puede ser producido mediante una capacidad criptográfica destruible asociada al payload.

El detalle exacto se fijará en implementación criptográfica, pero deben cumplirse simultáneamente:

1. Auditor puede verificar que el payload auditado es el mismo que llegará a Commit.
2. Un atacante no puede confirmar fácilmente un plaintext purgado usando artefactos históricos.
3. La capacidad de verificar contenido sensible puede destruirse durante PURGE.
...
# 63. Vault metadata and atomicity

SQLite y filesystem no comparten una transacción ACID única.

Por ello se define protocolo de staging.

## Creación

```text
1. create random payload_id
2. create key in KeyProvider
3. encrypt plaintext
4. write ciphertext to staging file
5. fsync
6. create payload_objects row status=STAGED
7. continue pipeline
8. on successful Commit:
      atomic rename staging -> payloads
      payload status -> ACTIVE
```

## Abort

Si Audit rechaza, Policy se invalida o expira:

```text
destroy key
delete staging ciphertext
...
```text
find payload status=STAGED
```

Para cada uno:

- si existe una transacción válida pendiente recuperable → reconciliar;
- si no → destruir key + borrar ciphertext + marcar ABORTED.
...
PURGE_PENDING
  |
  | key destroyed + payload removed/verified
  v
DESTROYED
```
...
[ ] un Patch no puede committearse dos veces
...
[ ] reconstrucción no revive payload purgado
